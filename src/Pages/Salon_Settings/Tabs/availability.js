...
function Availability() {
  ...
  const handleSubmit = () => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].flag == true && (data[i].STUTC == undefined || data[i].ETUTC == undefined)) {
        setCheck(true);
    ...
    setLoader(true)
    getAvailability(obj).then(({ data }) => {
      if (check) {
        toastMessage("success", "Availability Set/Updated")
      }
  ...
  return (
    <div>
      ...
      <button className='btn btnSlot mt-3' onClick={() => handleSubmit()}>
        {loader ?
